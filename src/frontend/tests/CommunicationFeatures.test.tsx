...
describe('SentenceStrip', () => {
  ...
  it('disables and pulses the speak button while speaking', () => {
    ...
    const speakButton = screen.getByLabelText('Speak sentence');
  ...
  it('reorders a chip after a pointer drag passes the activation distance', async () => {
    ...
    const secondSymbol = {
      ...
      id: 2,
      symbol_id: 2,
      symbol: { ...mockSymbol.symbol, id: 2, label: 'Bye' },
    };

    render(
      <SentenceStrip
        symbols={[mockSymbol, secondSymbol]}
        onRemove={vi.fn()}
        onClear={vi.fn()}
        onSpeak={vi.fn()}
        onReorder={onReorder}
    ...
    const firstChip = screen.getByText('Hello').closest('div');
    const secondChip = screen.getByText('Bye').closest('div');
    expect(firstChip).not.toBeNull();
    expect(secondChip).not.toBeNull();

    vi.spyOn(firstChip!, 'getBoundingClientRect').mockReturnValue({
      x: 0, y: 0, top: 0, right: 50, bottom: 50, left: 0, width: 50, height: 50,
      toJSON: () => {},
    });
    vi.spyOn(secondChip!, 'getBoundingClientRect').mockReturnValue({
      x: 50, y: 0, top: 0, right: 100, bottom: 50, left: 50, width: 50, height: 50,
      toJSON: () => {},
    });
    await user.pointer({ keys: '[MouseLeft>]', target: firstChip!, coords: { x: 10, y: 25 } });
    await user.pointer({ target: secondChip!, coords: { x: 75, y: 25 } });
    await user.pointer({ target: secondChip!, coords: { x: 80, y: 25 } });
    await new Promise(resolve => setTimeout(resolve, 0));
    await user.pointer({ keys: '[/MouseLeft]', target: secondChip!, coords: { x: 75, y: 25 } });
