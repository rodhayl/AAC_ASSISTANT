    ...
    return (
      <div className="max-w-4xl mx-auto p-6">
        <div className="mb-8 text-center">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-2">{t('symbolHunt.title')}</h1>
          <p className="text-gray-600">{t('symbolHunt.selectBoard')}</p>
        </div>

        {!loading && playableBoards.length === 0 && unplayableBoards.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-xl border border-gray-200">
            <p className="text-gray-500">{t('symbolHunt.noBoards')}</p>
          </div>
        ) : loading ? (
    ...
    return (
      <div className="max-w-md mx-auto p-6 text-center pt-20">
        <div className="mb-8">
          ...
          <h2 className="text-3xl font-bold text-gray-900 mb-2">{t('symbolHunt.greatJob')}</h2>
    ...
    <div className="h-[calc(100vh-4rem)] flex flex-col bg-gray-50">
      <div className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between shadow-sm z-10">
        <div className="flex items-center">
          <button onClick={() => setGameState('selecting')} aria-label={t('symbolHunt.back')} className="p-2 hover:bg-gray-100 rounded-full mr-4">
        ...
        <div className="flex items-center space-x-6">
          <div className="text-center">
            <div className="text-sm text-gray-500">{t('symbolHunt.score')}</div>
            <div className="text-2xl font-bold text-indigo-600">{score}</div>
          </div>
          {voiceEnabled && targetSymbol && (
            <button onClick={repeatInstruction} className="p-3 bg-indigo-100 text-indigo-600 rounded-full hover:bg-indigo-200 transition-colors" title={t('symbolHunt.repeat')}>
