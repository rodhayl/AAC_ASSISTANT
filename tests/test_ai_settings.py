...
class TestPrimaryAISettings:
    ...
    def test_update_ai_settings_groq(self, admin_user, admin_token):
        """Test updating AI settings to Groq"""
        response = client.put(
            "/api/settings/ai",
            headers={"Authorization": f"Bearer {admin_token}"},
            json={
        ...
        assert data["groq_model"] == "openai/gpt-oss-20b"
        assert data["groq_api_key"] == "gsk-test-key"

    def test_get_ai_settings_masks_groq_key_for_non_admin(
        self, admin_user, admin_token, regular_user, user_token
    ):
        """Non-admins see a masked Groq API key."""
        client.put(
            "/api/settings/ai",
            headers={"Authorization": f"Bearer {admin_token}"},
            json={"provider": "groq", "groq_api_key": "gsk-secret"},
        )
        response = client.get(
            "/api/settings/ai", headers={"Authorization": f"Bearer {user_token}"}
        )
        assert response.status_code == 200
        assert response.json()["groq_api_key"] == "********"
    ...
    def test_update_ai_settings_invalid_provider(self, admin_user, admin_token):
        ...
        assert "must be 'ollama'" in response.json()["detail"]
...
class TestSettingsValidationAndProviderModels:
    ...
    def test_get_groq_models_requires_api_key(self, admin_user, admin_token):
        response = client.get(
            "/api/settings/ai/models/groq",
            headers={"Authorization": f"Bearer {admin_token}"},
        )
        assert response.status_code == 400

    def test_get_groq_models_success(
        self, setup_test_db, test_db_session, admin_user, admin_token, monkeypatch
    ):
        from unittest.mock import AsyncMock, MagicMock
        ...
        test_db_session.add(
            AppSettings(setting_key="groq_api_key", setting_value="gsk-test-123")
        )
        test_db_session.commit()

        mock_provider = MagicMock()
        mock_provider.get_available_models = AsyncMock(
            return_value={"data": [{"id": "openai/gpt-oss-20b"}]}
        )
        monkeypatch.setattr(
            "src.api.routers.settings.GroqProvider", lambda **kwargs: mock_provider
        )
        response = client.get(
            "/api/settings/ai/models/groq",
            headers={"Authorization": f"Bearer {admin_token}"},
        ...
        assert response.status_code == 200
        assert response.json()["models"][0]["id"] == "openai/gpt-oss-20b"

    def test_get_groq_models_uses_unsaved_request_key(
        self, setup_test_db, test_db_session, admin_user, admin_token, monkeypatch
    ):
        from unittest.mock import AsyncMock, MagicMock
        ...
        test_db_session.add(
            AppSettings(setting_key="groq_api_key", setting_value="saved-key")
        )
        test_db_session.commit()
        ...
        monkeypatch.setattr(
            "src.api.routers.settings.GroqProvider",
            provider_factory,
        )
        response = client.get(
            "/api/settings/ai/models/groq",
