...
def test_backfill_downloads_missing_symbol_image(test_db_session, monkeypatch, tmp_path):
    ...
    assert symbol.image_path == "/uploads/symbols/arasaac_auto_1_2300.png"
    assert (uploads_dir / "symbols" / "arasaac_auto_1_2300.png").exists()
...
def test_backfill_uses_keyword_fallback_queries(test_db_session, monkeypatch, tmp_path):
    ...
    assert summary["downloaded"] == 1
    assert summary["failed"] == 0
    assert symbol.image_path == "/uploads/symbols/arasaac_auto_1_4901.png"
