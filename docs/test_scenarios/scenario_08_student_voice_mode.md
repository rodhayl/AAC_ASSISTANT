...
## Description
This historical end-to-end QA scenario recorded the complete voice mode workflow for students. It covers enabling voice mode, using speech-to-text for input, and using text-to-speech for output. The archived procedure used GUI automation; it is retained for historical provenance only and is not a supported test command.
...
- `src/frontend/src/pages/Settings/VoiceTab.tsx` - Voice mode UI
